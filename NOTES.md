# Project Objectives Checklist

- [x] Update `BACKEND_DESIGN.md` with near real-time, responsive mobile/desktop, containerization, and K8s deployment requirements.
- [x] Create `Dockerfile` to containerize the current application.
- [x] Create GitHub Action workflow for building and pushing the Docker image.
- [x] Deploy the application to the Kubernetes cluster using ArgoCD autopilot manifests.
- [x] Use the hostname `weather.waymack.org` for the application.
- [x] Verify functionality via `weather.waymack.org`.
- [x] Automate ArgoCD manifest update with new image tag in CI/CD pipeline.
- [ ] Implement near real-time backend (On-Demand model) as per `BACKEND_DESIGN.md`.
- [ ] Enhance mobile responsiveness and UI/UX.

